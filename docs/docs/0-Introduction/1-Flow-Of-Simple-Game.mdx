...
UrTurn calls your [room function](/docs/API/backend#functions) implementation of [`onRoomStart`](/docs/API/backend#onroomstart-required) to initialize the room state.
...
### 4. [Room functions](/docs/API/backend#functions) are [pure functions](/docs/api/backend#all-functions-are-pure-functions)

Notice how all of the implemented room functions ([`onRoomStart`](/docs/API/backend#onroomstart-required), [`onPlayerJoin`](/docs/API/backend#onplayerjoin-required), etc.) takes in the current [`roomState`](/docs/API/backend#roomstate) and several other arguments and returns a [`roomStateResult`](/docs/API/backend#roomstateresult).

Data should feel like a natural flow of transformations throughout time:
![pure functions diagram](./pure_function.png)

This makes it easier to understand, debug, and test your [room functions](/docs/API/backend#functions). More on [pure functions](/docs/api/backend#all-functions-are-pure-functions).
